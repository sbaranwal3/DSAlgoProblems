...
public class CheckIfAStringIsAValidSequenceFromRootToLeavesPathInABinaryTree {
    ...
    private static boolean isValid(TreeNode root, int[] arr, int i) {
        if(root.val!= arr[i])
            return false;
        if(i==arr.length-1)
            return root.left==null && root.right==null;
        return ((root.left!=null && isValid(root.left, arr, i+1))||(root.right!=null && isValid(root.right,arr,i+1)));
    }
    ...
    public static void main(String[] args) {
        ...
        int arr[] = {0,1,0,1};
        System.out.println("Is the sequence valid? "+isValidSequence(treeNode, arr));
    }
}
